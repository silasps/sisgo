      ...
      <main className="p-6">
        {!bases?.length ? (
          <div className="bg-white rounded-xl border border-dashed border-gray-300 p-10 text-center">
            ...
            <Link href="/superadmin/bases/nova" className="inline-flex px-4 py-2 bg-brand-500 text-white text-sm font-medium rounded-lg hover:bg-brand-600">
              + Criar primeira base
            </Link>
          </div>
        ) : (
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
              <p className="text-sm text-gray-500">{bases.length} {bases.length === 1 ? 'base' : 'bases'}</p>
            </div>
