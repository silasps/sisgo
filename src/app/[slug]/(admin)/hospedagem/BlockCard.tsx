...
export function BlockCard({ href, name, floorCount, roomCount, occupiedBeds, totalBeds, hold, actions }: Props) {
  const pct = totalBeds > 0 ? Math.round((occupiedBeds / totalBeds) * 100) : 0

  return (
    <div className="relative">
      {/* Ações ficam fora do <Link> de propósito — mesmo com stopPropagation,
          um clique que chega a um <a> aciona a barra de progresso de
          navegação (nextjs-toploader) antes do React conseguir interceptar.
          Só tirando o botão de dentro do link resolve de vez. */}
