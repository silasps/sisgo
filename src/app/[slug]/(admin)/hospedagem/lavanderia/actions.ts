...
export async function controlDevice(data: {
  ...
  const sb = createAdminClient()
  await controlMachineDevice(sb, data)
}

// ── Pagamento PIX (config da organização) ───────────────────────────────────

export async function upsertLaundryPaymentSettings(data: {
  organizationId: string
  environment: string
  accessToken: string | null      // null/vazio = mantém o token salvo
  defaultCustomerId: string | null
  webhookToken: string | null     // null/vazio = mantém o token salvo
  pixKey: string | null
  publicPaymentsEnabled: boolean
}) {
  const sb = createAdminClient()
  const { data: existing } = await sb.from('laundry_payment_settings')
    .select('access_token, webhook_token')
    .eq('organization_id', data.organizationId)
    .maybeSingle()
  ...
  const row = {
    ...
    default_customer_id: data.defaultCustomerId?.trim() || null,
    webhook_token:       data.webhookToken?.trim() || existing?.webhook_token || crypto.randomUUID().replace(/-/g, ''),
    pix_key:             data.pixKey?.trim() || null,
    public_payments_enabled: data.publicPaymentsEnabled,
    updated_at:          new Date().toISOString(),
  }
  const { error } = await sb.from('laundry_payment_settings').upsert(row)
  if (error) throw new Error(error.message)
}
