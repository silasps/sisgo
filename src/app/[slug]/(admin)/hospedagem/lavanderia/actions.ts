...
export async function checkMachinesOnline(machines: Array<{ id: string; device_ip: string | null; device_type: string | null }>): Promise<Record<string, boolean>> {
  const sb = createAdminClient()
  const deviceTypes = [...new Set(machines.map(m => m.device_type).filter(Boolean))] as string[]
...
export async function controlDevice(data: {
  ...
  const { device_ip: ip, device_type: deviceType } = machine as { device_ip: string | null; device_type: string | null }
  if (!ip) throw new Error('IP do dispositivo não configurado')
