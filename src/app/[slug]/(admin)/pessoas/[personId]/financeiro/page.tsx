...
type ChargeRow = { id: string; description: string; amount: number; due_date: string; status: string; reference_month: string | null; notes: string | null }
...
export default async function PessoaFinanceiroPage({ params }: Props) {
    ...
    <main className="p-4 md:p-6 space-y-5 max-w-2xl">
      ...
      {pending.length > 0 && (
        <section className="rounded-xl border border-gray-200 bg-white overflow-hidden">
          ...
          <div className="divide-y divide-gray-100">
            {pending.map(c => <ChargeItem key={c.id} charge={c} />)}
          </div>
        </section>
      )}
      ...
      {paid.length > 0 && (
        <section className="rounded-xl border border-gray-200 bg-white overflow-hidden">
          ...
          <div className="divide-y divide-gray-100">
            {paid.map(c => <ChargeItem key={c.id} charge={c} />)}
          </div>
        </section>
      )}
      ...
      {other.length > 0 && (
        <section className="rounded-xl border border-gray-200 bg-white overflow-hidden">
          ...
          <div className="divide-y divide-gray-100">
            {other.map(c => <ChargeItem key={c.id} charge={c} />)}
          </div>
        </section>
      )}
      ...
      {charges.length === 0 && (
        <div className="rounded-xl border border-dashed border-gray-300 p-10 text-center">
          <p className="text-gray-400 text-sm">Nenhuma cobrança registrada para esta pessoa.</p>
        </div>
      )}
    </main>
  )
