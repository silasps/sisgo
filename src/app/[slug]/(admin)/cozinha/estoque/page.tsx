  ...
  return (
    <>
      <Header
        title="Estoque"
        actions={
          <Link href={`/${slug}/cozinha`} className="text-sm text-gray-500 hover:text-gray-700 px-3 py-2 rounded-lg hover:bg-gray-100 transition-colors">
            ← Cozinha
          </Link>
        }
      />
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-xl border border-green-200 bg-green-50 overflow-hidden">
                <div className="border-b border-green-100 px-4 py-3">
                  <h2 className="text-sm font-semibold text-green-800">Registrar entrada</h2>
                </div>
                <form action={handleCreateStockEntry} className="space-y-2.5 p-4">
                  ...
                  <SbField name="supplier_name" label="Fornecedor" placeholder="Nome ou empresa" />
                  <div className="grid grid-cols-2 gap-2">
                    <SbField name="expiration_date" label="Validade" type="date" />
                    <SbField name="unit_cost" label="Custo unit." type="number" step="0.01" min="0" placeholder="R$" />
                  </div>
                  <SbField name="received_at" label="Recebido em" type="date" defaultValue={today} />
                  <SbField name="lot_code" label="Cód. lote" placeholder="Opcional" />
                  <button className="w-full rounded-lg bg-green-600 hover:bg-green-700 px-4 py-2 text-sm font-semibold text-white transition-colors">
                    + Registrar entrada
                  </button>
                </form>
              </div>
              ...
              <div className="rounded-xl border border-gray-200 bg-white overflow-hidden">
                <div className="border-b border-gray-100 px-4 py-3">
                  <h2 className="text-sm font-semibold text-gray-800">Saída / Perda / Ajuste</h2>
                </div>
                <form action={handleCreateStockMovement} className="space-y-2.5 p-4">
                  ...
                  <SbField name="reason" label="Motivo" placeholder="Ex: cardápio do dia" />
                  <SbField name="movement_date" label="Data" type="date" defaultValue={today} />
                  <button className="w-full rounded-lg bg-gray-900 hover:bg-gray-800 px-4 py-2 text-sm font-semibold text-white transition-colors">
                    Registrar movimento
                  </button>
                </form>
              </div>
              ...
              <details className="rounded-xl border border-gray-200 bg-white overflow-hidden">
                <summary className="px-4 py-3 text-sm font-semibold text-brand-600 hover:bg-brand-50 cursor-pointer transition-colors list-none">
                  + Cadastrar novo item
                </summary>
                <form action={handleCreateStockItem} className="space-y-2.5 p-4 border-t border-gray-100">
                  <SbField name="name" label="Nome *" required placeholder="Ex: Arroz branco" />
                  <div className="grid grid-cols-2 gap-2">
                    <SbField name="category" label="Categoria" placeholder="Ex: Grãos" />
                    <SbField name="unit" label="Unidade" placeholder="kg, un, L..." defaultValue="un" />
                  </div>
                  <div className="grid grid-cols-2 gap-2">
        ...
        {activeTab === 'fornecedores' && (
          <div className="grid gap-5 xl:grid-cols-[1fr_320px]">
              ...
              {(!suppliers || suppliers.length === 0) ? (
                ...
                <details key={sup.id} className="group rounded-xl border border-gray-200 bg-white overflow-hidden transition-all hover:border-brand-200 hover:shadow-sm">
                  <summary className="flex items-center justify-between gap-3 px-4 py-4 cursor-pointer list-none">
                    ...
                    <span className="text-xs text-brand-500 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">Editar →</span>
                  </summary>
            ...
            <div className="rounded-xl border border-gray-200 bg-white overflow-hidden self-start">
              <div className="border-b border-gray-100 px-4 py-3">
                <h2 className="text-sm font-semibold text-gray-800">Novo fornecedor</h2>
              </div>
              <form action={handleCreateStockSupplier} className="space-y-2.5 p-4">
                <SbField name="name" label="Nome *" required placeholder="Ex: Distribuidora Silva" />
                <SbField name="description" label="Descrição" placeholder="O que fornece?" />
                <SbField name="contact_email" label="E-mail" type="email" />
                <div>
                  <span className="mb-1 block text-xs font-medium text-gray-600">Telefone</span>
                  <InternationalPhoneField countryName="contact_country_code" phoneName="contact_phone" defaultCountryIso="BR" defaultPhone="" />
                </div>
                <SbField name="cnpj" label="CNPJ" placeholder="00.000.000/0000-00" />
                <SbField name="address" label="Endereço" placeholder="Rua, cidade..." />
                <SbField name="notes" label="Notas" placeholder="Observações..." />
                <button className="w-full rounded-lg bg-brand-500 hover:bg-brand-600 px-4 py-2 text-sm font-semibold text-white transition-colors">
...
function SbSelect({ label, name, options }: { label: string; name: string; options: Array<[string, string]> }) {
  return (
