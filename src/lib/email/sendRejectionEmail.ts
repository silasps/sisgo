...
import { getRejectionDict, type EmailLang } from '@/lib/i18n/emails'
            ...
            ${p.decisionNote ? `<table width="100%" cellpadding="0" cellspacing="0" style="background:#f9fafb;border-radius:10px;border:1px solid #e5e7eb;">
              <tr>
                <td style="padding:20px 24px;">
                  <p style="margin:0 0 8px;font-size:13px;font-weight:600;color:#4b5563;text-transform:uppercase;letter-spacing:0.05em;">${d.leaderWordTitle}</p>
        ...
        <tr>
          <td style="background:#f9fafb;border-top:1px solid #e5e7eb;padding:24px 40px;text-align:center;">
            <p style="margin:0;font-size:13px;color:#6b7280;">
              ${d.contact} <a href="mailto:${p.replyTo}" style="color:#374151;">${p.replyTo}</a>
            </p>
          </td>
        </tr>
      </table>
...
export async function sendRejectionEmail(params: Params): Promise<void> {
  ...
  const subject = getRejectionDict(lang).subject.replace('{school}', params.schoolName)
